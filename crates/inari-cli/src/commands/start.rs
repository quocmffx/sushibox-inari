use anyhow::Result;
use inari_core::{job::JobObject, paths::InariPaths, process::spawn_service};
use tracing::warn;
...
pub async fn run() -> Result<()> {
    ...
    for desc in &descriptors {
        ...
        match spawn_service(desc) {
            ...
            Err(e) => println!("  [ERR] {} — {e}", desc.kind.display_name()),
        }
    }
    ...
    println!("Started {started} service(s), skipped {skipped} (missing binary).");
